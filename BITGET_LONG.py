...
# ASCII Art
ascii_art = """
  ____ _____ _______ _____ ______ _______   _             __  __ __ 
 |  _ \_   _|__   __/ ____|  ____|__   __| | |           /_ |/_ /_ |
 | |_) || |    | | | |  __| |__     | |    | |      __   _| | | || |
 |  _ < | |    | | | | |_ |  __|    | |    | |      \ \ / / | | || |
 | |_) || |_   | | | |__| | |____   | |    | |____   \ V /| |_| || |
 |____/_____|  |_|  \_____|______|  |_|    |______|   \_/ |_(_)_||_|
...
# Function to open a long position on Bitget
async def place_long_position_async(ticker, investment_usd=1, leverage=40):
    ...
    try:
        ...
        close_result = await close_long_position_with_limit(ticker, leverage)
...
async def close_long_position_with_limit(symbol, leverage):
...
async def handle_message(message: Message):
    ...
    if found_tickers:
        ...
        for ticker in found_tickers:
            ...
            await message.answer(f"Bitget Recognized ticker: {ticker}. Setting long position with SL and setting close order...")
            task = asyncio.create_task(place_long_position_async(ticker))
            tasks.append(task)
        ...
        for ticker, result in zip(found_tickers, results):
            if isinstance(result, Exception):
                logging.error(f"Error opening position for {ticker}: {str(result)}")
                await message.answer(f"Failed to set position for {ticker}. Error: {str(result)}")
            else:
                logging.info(f"Result from Bitget API for {ticker}: {result}")
                if 'error' in result:
                    await message.answer(f"Bitget Failed to open position for {ticker}: {result['error']}")
                else:
                    await message.answer(f"Bitget Position for {ticker} opened successfully with SL set and close order placed.")
                    # Check and send unrealized P&L immediately after opening a position
                    position = await check_position(ticker)
                    if position:
                        unrealized_pl = position.get('unrealizedPL', '0')
                        await message.answer(f"[{datetime.now().strftime('%d.%m.%Y %H:%M')}]\nBitget Current unrealized P&L for {ticker}: {unrealized_pl}")
                    else:
                        await message.answer(f"[{datetime.now().strftime('%d.%m.%Y %H:%M')}]\nBitget Could not fetch position data for {ticker} to get unrealized P&L.")
...
async def send_unrealized_pl(chat_id):
    while True:
        try:
            ...
            logging.error(f"Bitget Error fetching or sending unrealized P/L: {e}", exc_info=True)
